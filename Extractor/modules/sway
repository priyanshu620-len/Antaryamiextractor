...
async def cmd_sq_handler(client, message: Message):
    ...
    # Save batches to state
    USER_SESSIONS[user_id] = batches

    # Build Course Index TXT
...
@app.on_message(filters.text & filters.private & ~filters.command(["sq", "sway", "start"]))
async def process_course_selection(client, message: Message):
    user_id = message.from_user.id
    ...
    status_msg = await message.reply_text(f"⏳ **Extracting:** `{batch_title}`\n\n_Fetching topics & class links..._")
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for t_idx, (topic, classes) in enumerate(zip(topics, all_topic_classes), 1):
            ...
            # Rate-limit status message edits to avoid Telegram FloodWait
            now = time.time()
